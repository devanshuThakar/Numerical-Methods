...
for i=2:length(t)
    ...
    R = T(i-1,2:(Lu-Ll)/dx);
    R(1) = R(1) + lambda*T(i,1);
    R((Lu-Ll)/dx-1) = R((Lu-Ll)/dx-1) + lambda*T(i,(Lu-Ll)/dx + 1);
    
    f = zeros((Lu-Ll)/dx-1) + (1+2*lambda);
    e = zeros((Lu-Ll)/dx-2) + (-1*lambda);
    g = zeros((Lu-Ll)/dx-2) + (-1*lambda);
...
title("Plot of Temperature(T) vs. distance (x), by Implicit method");
xlabel("x(cm)");
